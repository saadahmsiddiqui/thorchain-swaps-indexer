...
export async function storeActionMaxGas(
    ...
    const query = `INSERT INTO mayachain.action_max_gas (${cols}) VALUES ($1,$2,$3,$4)`;
...
export async function getActionMaxGas(db: Pool, action_id: string): Promise<ActionMaxGas[]> {
    const query = `SELECT * FROM mayachain.action_max_gas WHERE action_id = $1`;
...
export async function updateActionMaxGas(
    ...
    const query = `
    UPDATE mayachain.action_max_gas SET
      asset = $2,
      amount = $3,
      decimals = $4
