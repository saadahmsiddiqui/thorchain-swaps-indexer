...
export async function storeTransactionOutHash(
    ...
    const query = `INSERT INTO mayachain.tx_out_hashes (${cols}) VALUES ($1,$2)`;
...
export async function getTransactionOutHashes(
    ...
    const query = `SELECT * FROM mayachain.tx_out_hashes WHERE tx_id = $1`;
...
export async function updateTransactionOutHash(
    ...
    const query = `UPDATE mayachain.tx_out_hashes SET out_hash = $2 WHERE id = $1 AND tx_id = $3`;
