...
export async function store(
    ...
    const query = `INSERT INTO mayachain.indexed_hashes(${cols}) VALUES (${placeholders}) ON CONFLICT DO NOTHING;`;
...
export async function get({
    ...
    const query = `SELECT * FROM mayachain.indexed_hashes ${where}`;
...
export async function getHavingState({
    ...
    const dbReadWrite = await getClient('r');
    const query = `SELECT * FROM mayachain.indexed_hashes WHERE state = $1 LIMIT $2`;
...
export async function getHavingStates({
    ...
    const query = `SELECT * FROM mayachain.indexed_hashes WHERE state IN (${_states}) LIMIT $1`;
...
export async function updateState(hash: string, state: string): Promise<boolean> {
    const dbReadWrite = await getClient('rw');
    const query = `UPDATE mayachain.indexed_hashes SET state = $1 WHERE hash = $2`;
