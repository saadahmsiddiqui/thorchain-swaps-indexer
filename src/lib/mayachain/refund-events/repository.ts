...
export async function get(id: string): Promise<RefundEvent | null> {
    ...
    const query = `SELECT protocol, height, id, coin, reason FROM mayachain.refund_events WHERE id = $1;`;
...
export async function store(event: RefundEvent): Promise<boolean> {
    ...
    const query = `INSERT INTO mayachain.refund_events (${cols}) VALUES (${placeholders}) ON CONFLICT DO NOTHING;`;
