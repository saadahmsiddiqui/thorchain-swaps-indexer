...
export async function storeTransactionBaseInfo(
  ...
  const query = `INSERT INTO thorchain.tx_base_info (${cols}) VALUES ($1,$2,$3,$4,$5) RETURNING id`;
...
export async function getTransactionBaseInfo(
  ...
  const query = `SELECT * FROM thorchain.tx_base_info WHERE tx_id = $1`;
...
export async function updateTransactionBaseInfo(
  ...
  const query = `
    UPDATE thorchain.tx_base_info SET
      consensus_height = $2,
      finalised_height = $3,
      updated_vault = $4,
      outbound_height = $5
