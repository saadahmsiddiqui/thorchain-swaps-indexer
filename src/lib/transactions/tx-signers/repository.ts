...
export async function storeTransactionSigner(
  ...
  const query = `INSERT INTO thorchain.tx_signers (${cols}) VALUES ($1,$2)`;
...
export async function getTransactionSigners(db: Pool, tx_id: string): Promise<TransactionSigner[]> {
  const query = `SELECT * FROM thorchain.tx_signers WHERE tx_id = $1`;
...
export async function updateTransactionSigner(
  ...
  const query = `UPDATE thorchain.tx_signers SET signer = $2 WHERE id = $1 AND tx_id = $3`;
