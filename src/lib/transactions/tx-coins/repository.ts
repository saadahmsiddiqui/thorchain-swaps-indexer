...
export async function storeTransactionCoins(
  ...
  const query = `INSERT INTO thorchain.tx_detail_coins (${cols}) VALUES ($1,$2,$3,$4)`;
...
export async function getTransactionCoins(
  ...
  const query = `SELECT * FROM thorchain.tx_detail_coins WHERE tx_id = $1 AND coin_type = $2`;
...
export async function updateTransactionCoins(
  ...
  const query = `
    UPDATE thorchain.tx_detail_coins SET
      coin_type = $2,
      asset = $3,
      amount = $4
